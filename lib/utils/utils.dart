...
class Utils {
  ...
  static Future<Map<String, dynamic>> statisticTraveling(
      List<Position> positions) async {
    return {
      'velocity': await Utils.calculateVelocity(positions),
      // 'from': await Utils.getDistrict(positions.first),
      // 'to': await Utils.getDistrict(positions.last),
      'distance': await Utils.calculateDistance(positions)
    };
  }

  static Future<double> calculateVelocity(List<Position> positions) async {
    return (await Utils.calculateDistance(positions)) /
        positions.last.timestamp
            .difference(positions.first.timestamp)
            .inSeconds;
  }

  static Future<String> getDistrict(Position p) {}

  static Future<double> calculateDistance(List<Position> positions) async {
    double distance = 0.0;
